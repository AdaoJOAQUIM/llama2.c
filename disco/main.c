...
/* ---------------- the SEED format ----------------
 * A .seed file is a COMPLETE model: header + the K trainable fp32 values.
 * Everything else is regenerated by re-running the architecture's build() with
 * the recorded integer seed, which is exactly what the training run did.  The
 * file is therefore self-describing and self-sufficient -- `infer --loadseed`
 * never opens the dense checkpoint.
 * Compression ratio is measured as (bytes of the dense .bin) / (bytes of this
 * file), both read from the filesystem.  Nothing is estimated. */
typedef struct { char magic[4]; int32_t seed, fseed; float frac;
                 int64_t ntot, k; char arch[64]; char cfg[96]; } SeedHdr;
static void seed_write(const char*p,const char*archname,const char*cfgs,
                       int seed,int fseed,float frac){
  SeedHdr h; memset(&h,0,sizeof h); memcpy(h.magic,"SD01",4);
  h.seed=seed; h.fseed=fseed; h.frac=frac;
  h.ntot=params_count(); h.k=frozen_trainable();
  snprintf(h.arch,64,"%s",archname); snprintf(h.cfg,96,"%s",cfgs);
  float *buf=(float*)malloc(sizeof(float)*(size_t)h.k); frozen_gather(buf);
  FILE*f=fopen(p,"wb"); fwrite(&h,sizeof h,1,f); fwrite(buf,4,(size_t)h.k,f); fclose(f);
  free(buf);
}
static void seed_read(const char*p){
  FILE*f=fopen(p,"rb"); if(!f){fprintf(stderr,"no seedfile %s\n",p);exit(1);}
  SeedHdr h; if(fread(&h,sizeof h,1,f)!=1){fprintf(stderr,"seed hdr\n");exit(1);}
  if(memcmp(h.magic,"SD01",4)){fprintf(stderr,"bad seed magic\n");exit(1);}
  if(h.ntot!=params_count()){fprintf(stderr,"seed ntot %lld vs %lld\n",(long long)h.ntot,params_count());exit(1);}
  if(h.k!=frozen_trainable()){fprintf(stderr,"seed k %lld vs %lld -- frac/fseed mismatch\n",
                                      (long long)h.k,frozen_trainable());exit(1);}
  float *buf=(float*)malloc(sizeof(float)*(size_t)h.k);
  if(fread(buf,4,(size_t)h.k,f)!=(size_t)h.k){fprintf(stderr,"seed payload\n");exit(1);}
  fclose(f); frozen_apply(buf); free(buf);
}
static long file_bytes(const char*p){ FILE*f=fopen(p,"rb"); if(!f) return -1;
...
/* Mean bpb as a function of POSITION in the window, on the same val offsets.
 * A single scalar hides the shape: a model can be fine predicting the next byte
 * of a word and useless at exploiting 200 bytes of context, and both collapse
 * into one number.  If compression destroys long-range structure first, the
 * per-position curve flattens while the scalar barely moves. */
static void eval_bypos(Arch*A,Cfg*c,int nb,int B,int T,double*out,int nbuck){
  int sv=g_train; g_train=0; params_invalidate_q();
  long off[256]; val_offsets(off,nb,T*B+2);
  int *x=malloc(sizeof(int)*B*T),*y=malloc(sizeof(int)*B*T);
  double *sum=calloc(nbuck,sizeof(double)); long long *cnt=calloc(nbuck,sizeof(long long));
  int V=c->vocab, per=T/nbuck;
  for(int i=0;i<nb;i++){
    arena_reset(); tape_reset();
    fill_batch(off[i],B,T,x,y,T);
    Tensor*lg=A->fwd(c,x,B,T,0,0);
    for(int b=0;b<B;b++)for(int t=0;t<T;t++){
      const float*r=lg->d+(size_t)(b*T+t)*V;
      float mx=r[0]; for(int q=1;q<V;q++) if(r[q]>mx) mx=r[q];
      double s=0; for(int q=0;q<V;q++) s+=exp((double)r[q]-mx);
      double nll=-((double)r[y[b*T+t]]-mx-log(s));
      int k=t/per; if(k>=nbuck) k=nbuck-1;
      sum[k]+=nll; cnt[k]++;
    }
  }
  for(int k=0;k<nbuck;k++) out[k]= cnt[k]? sum[k]/cnt[k]/0.6931472 : 0.0;
  free(x);free(y);free(sum);free(cnt); g_train=sv;
}
...
int main(int argc,char**argv){
  ...
  float frozen=1.0f; int fseed=7, fmode=0; const char*loadseed=NULL,*seedout=NULL;
  ...
  for(int i=2;i<argc;i++){
    ...
    else if(ARG("--frozen")) frozen=atof(argv[++i]);     /* trainable FRACTION, 1.0 = normal */
    else if(ARG("--fseed")) fseed=atoi(argv[++i]);       /* which coordinates are trainable */
    else if(ARG("--fmode")) fmode=atoi(argv[++i]);       /* 0 random, 1 gains-first */
    else if(ARG("--loadseed")) loadseed=argv[++i];       /* run entirely from a .seed file */
    else if(ARG("--seedout")) seedout=argv[++i];
  ...
  /* frozen/fseed are also reachable as knobs so that model.lab -- which only
     speaks --knob -- can pre-register and run these variants itself. */
  if(frozen>=1.0f) frozen=cfg_get(&c,"frozen",1.0f);
  if(fseed==7)     fseed=cfg_geti(&c,"fseed",7);
  if(fmode==0)     fmode=cfg_geti(&c,"fmode",0);
  /* lrx overrides --lr.  Needed because a model with 0.5% of its coordinates
     trainable may well want a different step size, and comparing it at the
     dense protocol's LR would handicap the very thing under test. */
  { float kl=cfg_get(&c,"lrx",0.0f); if(kl>0) lr=kl; }
  ...
  if(!strcmp(mode,"train")){
    arena_init(arena);
    A->build(&c); opt_init();
    frozen_build(frozen,(uint32_t)fseed,fmode);
    if(frozen_active())
      fprintf(stderr,"[frozen] trainable %lld / %lld (%.4f%%)  fseed=%d\n",
              frozen_trainable(),params_count(),100.0*frozen_trainable()/(double)params_count(),fseed);
    int step0=0;
    ...
    double ratio=0;
    char sodef[600];
    if(frozen_active()&&!seedout){ snprintf(sodef,sizeof sodef,"%s.seed",ckpt); seedout=sodef; }
    if(frozen_active()&&seedout){
      char cfgs[96]; snprintf(cfgs,sizeof cfgs,"d%d/l%d/h%d/kv%d/f%d/v%d/tie%d",
                              c.dim,c.n_layers,c.n_heads,c.n_kv_heads,c.hidden_dim,c.vocab,c.tie);
      seed_write(seedout,archname,cfgs,seed,fseed,frozen);
      long db=file_bytes(ckpt), sb2=file_bytes(seedout);
      ratio = sb2>0? (double)db/(double)sb2 : 0;
      fprintf(stderr,"[seed] %s  %ld B  vs dense %ld B  ratio %.1fx\n",seedout,sb2,db,ratio);
    }
    ...
    if(jsonp){ FILE*j=fopen(jsonp,"w");
      fprintf(j,"{\"arch\":\"%s\",\"params\":%lld,\"stored_bytes\":%.0f,\"train_loss\":%.5f,"
                ...
                "\"frozen_frac\":%.6f,\"trainable\":%lld,\"seed_bytes\":%ld,\"dense_bytes\":%ld,\"ratio\":%.2f}\n",
        archname,params_count(),params_bytes(),lastloss,vl,vl/0.6931472,ttrain,steps,B,T,lr,seed,
        (long long)steps*B*T,g_tpb,g_ffast,
        (double)frozen,frozen_active()?frozen_trainable():params_count(),
        seedout?file_bytes(seedout):-1L,file_bytes(ckpt),ratio);
      fclose(j); }
    return 0;
  }
  ...
  if(!strcmp(mode,"infer")){
    ...
    arena_init(16ull<<20);
    double tload=now_sec();
    A->build(&c);
    if(loadseed){ frozen_build(frozen,(uint32_t)fseed,fmode); seed_read(loadseed); }
    else ckpt_load(ckpt);
    tload=now_sec()-tload;   /* regeneration is part of the cost of the format */
    ...
    if(jsonp){ FILE*j=fopen(jsonp,"w");
      fprintf(j,"{\"arch\":\"%s\",\"params\":%lld,\"stored_bytes\":%.0f,\"wbytes_per_tok\":%lld,"
                ...
                "\"frac_fast_decode\":%.4f,\"load_sec\":%.4f,\"artifact_bytes\":%ld}\n",
        archname,params_count(),params_bytes(),wu,wb,ww,sb,fl,best_tps,rss,maxdiff,gen,KOUT,ffast,
        tload,file_bytes(loadseed?loadseed:ckpt));
      fclose(j); }
  ...
  if(!strcmp(mode,"eval")){
    arena_init(arena); A->build(&c);
    if(loadseed){ frozen_build(frozen,(uint32_t)fseed,fmode); seed_read(loadseed); }
    else ckpt_load(ckpt);
    double vl=eval_val(A,&c,nvalb,B,T,KOUT,TAU);
    printf("val_loss %.5f bpb %.5f\n",vl,vl/0.6931472);
    if(cfg_geti(&c,"bypos",0)){
      double bp[16]; eval_bypos(A,&c,nvalb,B,T,bp,16);
      printf("bypos");
      for(int k=0;k<16;k++) printf(" %.5f",bp[k]);
      printf("\n");
    }
    if(jsonp){ FILE*j=fopen(jsonp,"w");
      fprintf(j,"{\"arch\":\"%s\",\"val_loss\":%.5f,\"val_bpb\":%.5f,\"data\":\"%s\","
                "\"artifact\":\"%s\",\"artifact_bytes\":%ld}\n",
              archname,vl,vl/0.6931472,data,loadseed?loadseed:ckpt,
              file_bytes(loadseed?loadseed:ckpt)); fclose(j); }
    return 0;
  }
  /* Is the SEED format LOSSLESS with respect to the model it claims to store?
     Rebuild from (seed, K values) and compare coordinate-by-coordinate against
     the dense checkpoint.  Anything but 0.0 means the ratio is being quoted for
     an artifact that is not the measured model. */
  if(!strcmp(mode,"seedcheck")){
    arena_init(arena); A->build(&c);
    frozen_build(frozen,(uint32_t)fseed,fmode);
    if(!loadseed){ fprintf(stderr,"seedcheck needs --loadseed\n"); return 1; }
    seed_read(loadseed);
    long long ntot=params_count();
    float *mine=(float*)malloc(sizeof(float)*(size_t)ntot);
    { long long k=0; for(int i=0;i<g_nparams;i++) for(int j=0;j<g_params[i]->n;j++) mine[k++]=g_params[i]->d[j]; }
    FILE*f=fopen(ckpt,"rb"); if(!f){fprintf(stderr,"no dense ckpt %s\n",ckpt);return 1;}
    int magic,np; if(fread(&magic,4,1,f)!=1||fread(&np,4,1,f)!=1) return 1;
    double maxd=0; long long k=0, ndiff=0;
    for(int i=0;i<np;i++){ int n; if(fread(&n,4,1,f)!=1) return 1;
      float*tmp=(float*)malloc(sizeof(float)*n);
      if(fread(tmp,4,n,f)!=(size_t)n) return 1;
      for(int j=0;j<n;j++){ double d=fabs((double)tmp[j]-mine[k]); if(d>maxd) maxd=d; if(d!=0) ndiff++; k++; }
      free(tmp); }
    fclose(f);
    long db=file_bytes(ckpt), sb2=file_bytes(loadseed);
    printf("{\"max_abs_diff\":%.6e,\"n_differing\":%lld,\"ntot\":%lld,"
           "\"dense_bytes\":%ld,\"seed_bytes\":%ld,\"ratio\":%.3f,\"lossless\":%s}\n",
           maxd,ndiff,ntot,db,sb2,(double)db/(double)sb2,maxd==0.0?"true":"false");
    return maxd==0.0?0:1;
  }
  fprintf(stderr,"usage: disco {train|infer|eval|seedcheck|memtest|gradcheck|list} [opts]\n"); return 1;
}
